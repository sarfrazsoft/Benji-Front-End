...
export class ParticipantHintActivityComponent extends BaseActivityComponent {
  ...
  participantHasSubmitted() {
    return this.activityState.activity_status.submitted_users.indexOf(this.activityState.your_identity.id) > -1;
  }

  participantHasVoted() {
    return this.activityState.activity_status.voted_users.indexOf(this.activityState.your_identity.id) > -1;
  }

  public submitHintWord() {
    if (!this.participantHasSubmitted() && this.hintWord.value !== undefined) {
      this.sendMessage.emit({'event': 'submit_word', 'word': this.hintWord.value});
    }
  }
