...
export class GenericRoleplayComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const salesmenUsers = this.data.joined_users.filter(el => {
      return salesmen.find(role => el.id === role.benjiuser_id);
    });

    // salesmen have been found sargent
    console.log(salesmenUsers);
    ...
    questionList.forEach(question => {
      ...
      responsesForQuestion.forEach(res => {
        ...
        userGroup.usergroupuser_set.forEach(user => {
          const fbReceiever = salesmenUsers.find(u => user.user.id === u.id);
          if (fbReceiever) {
            assessments.push({
              user: fbReceiever,
    ...
    // this.questions = salesmanFeedback;
  }
...
// one way to create this data structure is by
// iterating over all of the questions
//   finding the text of each question
//   finding the user this question was asked to
//   is that possible??
